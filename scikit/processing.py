...
# column 16 (tuition fees up to date) seems to have a big impact on the accuracy (about 0.05)
...
# define some values for each hyperparameter as a starting point
param_grid = {
    'hidden_layer_sizes': [(100,), (100, 100), (50,), (10,), (20, 20,)], # doesn't seem to matter much
    'solver': ['adam'],
    'max_iter': [500],
    'alpha': [0.1,0.2, 0.5, 0.01], # between 0.1 and 0.5 seems to be the ok
    'learning_rate_init': [0.01],
}

grid_search = GridSearchCV(estimator=multilayer_perceptron, param_grid=param_grid, cv=10, scoring='accuracy', n_jobs=-1, verbose=1)
grid_search.fit(X_train, y_train)
